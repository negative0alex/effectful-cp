...
{-# LANGUAGE GADTs #-}
module Staging.IncrementalOptimisation2 where
import Data.Kind
...
-- attempt to introduce let-bindings; fail because of direct-style
...
composeTrans' t1 t2 = SearchTransformer' 
  ...
  , leftTs' = case (leftTs' t1, leftTs' t2) of 
      ...
      (f1, f2) -> TransState $ \ts -> let_ ts $ \tsL tsR -> 
          pairP (getTrans f1 tsL) (getTrans f2 tsR)
