...
import CPSolve (CPSolve, pattern Add, pattern Dynamic, pattern NewVar, dynamic, exist, in_domain, (@\=), (@\==), (@+), (@=))
import Control.Monad.Free (Free (..), MonadFree (wrap))
import Data.Sequence (Seq)
import Effects (Sub (..), Void, unitr, wrapF, (:+:) (..), pattern Other)
import NonDet (NonDet (..), fail, pattern Fail, pattern (:|:), try)
import Queues (Queue (..))
