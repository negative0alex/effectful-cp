...
-- import Solver(Solver(..), SolverEffect (..))
...
-- -- projectSolve :: forall solver sig a.(Solver solver, CPSolve solver `Sub` sig) =>
-- --   Free sig a -> Maybe (CPSolve solver (Free sig a))
-- -- projectSolve (Free s) = prj s
-- -- projectSolve _ = Nothing

-- pattern Dynamic :: forall solverE sig a. (solverE `Sub` sig, SolverEffect solverE) =>
--   (Solv solverE) (Free sig a) -> Free sig a
...
-- pattern NewVar k <- (project @solver -> Just (NewVar' k))
